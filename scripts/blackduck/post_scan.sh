...
if [ $CODEBUILD_BUILD_SUCCEEDING -eq 1 ]; then
  ...
  color="#31AD72"
  header="Success"
  text="Blackduck Scan for Access Checkout build *No.${CODEBUILD_BUILD_NUMBER}* for *${CODEBUILD_SOURCE_VERSION}* for version *${SDK_VERSION}* has completed successfully."
  ...
  color="#EA5E1A"
  header="Failed"
  text="Blackduck Scan for Access Checkout build *No.${CODEBUILD_BUILD_NUMBER}* for *${CODEBUILD_SOURCE_VERSION}* for version *${SDK_VERSION}* has failed."
  location="No report generated"
fi

payload="{
	\"channel\":\"#hydra-blackduck\",
	\"username\":\"Access Checkout Blackduck Scan\",
	\"icon_emoji\": \"${emoji}\",
	\"attachments\": [
		{
			\"color\": \"${color}\",
